    ...
    **SUMMARY**
    ...
    def __init__(self, camera_index = -1, prop_set = {}, threaded = True, calibrationfile = ''):
        ...
        * *calibrationfile* - A calibration file to load.
        ...
        self.capture = cv.VideoCapture(camera_index) #This fixes bug with opencv not being able to grab frames from webcams on linux
        self.index = camera_index
        if "delay" in prop_set:
            time.sleep(prop_set['delay'])

        if platform.system() == "Linux" and ("height" in prop_set or self.capture.read() == False):
    ...
    **EXAMPLE**
    ...
    def getAllProperties(self):
        ...
        **EXAMPLE**
        ...
        """
        props = {}
        for p in list(self._properties.keys()):
            props[p] = self.getProperty(p)
